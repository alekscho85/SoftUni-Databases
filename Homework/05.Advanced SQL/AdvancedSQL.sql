...
SELECT t.Name AS [Town], d.Name AS [Department], Count(e.EmployeeID) AS [Employess count]
FROM Employees e
JOIN Departments d
ON e.DepartmentID = d.DepartmentID
JOIN Addresses a
...
SELECT 
	CONVERT(varchar, GETDATE(), 104) + ' ' + RIGHT(CONVERT(varchar, GETDATE(), 113), 12) 
	AS [Date Time]
...
SELECT d.Name, e.JobTitle, AVG(e.Salary) AS[Average Salarry]
FROM Employees e
JOIN Departments d
ON e.DepartmentID = d.DepartmentID
...
SELECT d.Name AS[Department], e.JobTitle, MIN(e.FirstName) AS[First Name], MIN(e.Salary) AS[Min Salary]
FROM Employees e
JOIN Departments d
ON e.DepartmentID = d.DepartmentID
GROUP BY d.Name, e.JobTitle
...
SELECT TOP 1 t.Name, COUNT(e.EmployeeID) AS[Numbers Of Employees]
FROM Employees e
JOIN Departments d
ON e.DepartmentID = d.DepartmentID
JOIN Addresses a
...
SELECT
	t.Name, COUNT(DISTINCT m.EmployeeID) AS[Numbers Of Managers]
FROM Employees e
JOIN Employees m 
ON e.ManagerID = m.EmployeeID
JOIN Addresses a
...
GO
--32.Start a database transaction, delete all employees from the 'Sales' department along with all dependent records from the pother tables. At the end rollback the transaction.
BEGIN TRAN
